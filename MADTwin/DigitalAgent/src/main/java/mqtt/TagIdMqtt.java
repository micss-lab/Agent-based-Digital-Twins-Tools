...
public class TagIdMqtt {
    Logger logger = Logger.getLogger(TagIdMqtt.class.getName());
        ...
        public void messageArrived(String topic, MqttMessage message) {
            ...
            synchronized (lock) {
                try {
                        ...
                        if (jsonObject.getJSONObject(i).getInt("tagId") == Integer.parseInt(tagId, 16)) {
                            if (Integer.parseInt(tagId, 16) == json.getInt("tagId")) {
                                if (json.getBoolean("success")) {
                                    ...
                                    yawAngle=(float) Math.toDegrees(orientation.getFloat("yaw"));
                                    lock.notify();
                    ...
                    logger.info("Malformed message received: " + message);
                }
            }
        }
